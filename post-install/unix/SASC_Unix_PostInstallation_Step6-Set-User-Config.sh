#!/bin/ksh
########################################################################################################################
# Script: SASC_Unix_PostInstallation_Step6-Set-User-Config.sh
#
...
#                           Piotr Ulaszewski - 07/02/2023 - Changement des mots de passe geres par ansible directement
#                           Piotr Ulaszewski - 09/03/2023 - La partie AIX a été ajouté a nouveau de la version post install par HPSA
#                           Piotr Ulaszewski - 31/03/2023 - Changement des mots de passe commentés sur la partie AIX, gerge directement par ansible
#                           Piotr Ulaszewski - 03/04/2023 - Commentaire sur la creation du compte exp_cac (mail pour consignes envoye)
#                           Piotr Ulaszewski - 26/05/2023 - Commentaire sur la creation du compte exp_cac pour Linux supprimé (suite a la conf BotFactory)
#                                                         - pour etre ISO PROD, ce compte actuellement ne peut pas être utilisé
#
...
#                echo "Changement mdp cac root admsrv"
#                FIC_MDP=/tmp/pwd.csv
#
#                echo "OS;HOSTNAME;USER;PASSWORD" > $FIC_MDP
#                for user in root cac admsrv
#                do
...
#                        if [ $? = 0 ]
#                        then
#                            echo "${OS};${HOSTNAME};${user};${MDP}" >> $FIC_MDP
#                        else
#                            echo "${OS};${HOSTNAME};${user};BAD PASSWORD" > $LOG
#                            echo "${OS};${HOSTNAME};${user};BAD PASSWORD" >> $FIC_MDP >&2 && exit 5
#                        fi
#                    else
#                        echo "${OS};${HOSTNAME};${user};USER NOT FOUND" > $LOG
#                        echo "${OS};${HOSTNAME};${user};USER NOT FOUND" >> $FIC_MDP >&2 && exit 4
#                    fi
#                done
        ;;		
