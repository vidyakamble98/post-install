#!/bin/ksh
########################################################################################################################
# Script: SASC_Unix_PostInstall_Step2-Set-Security-Config.ksh
#
...
#                           Piotr Ulaszewski - 06/02/2023 - Ajout de exit 0 pour signaler un succes
#                           Piotr Ulaszewski - 08/03/2023 - Mise a jour des vmware tools ajouté pour Linux 8 (porté de la version pour Linux 7)
#                           Piotr Ulaszewski - 08/03/2023 - Patch CVE pour Linux 7 porté de l'ancienne post-install HPSA
#                           Piotr Ulaszewski - 30/03/2023 - Info pas de patch CVE sur AIX
#                           Piotr Ulaszewski - 13/11/2023 - Revue - patch CVE Linux 8, microcode_ctl Linux 7/8, sssd Linux 7
#
...
if [[ $FOURNISSEUR != "CONSER" && $FOURNISSEUR != "PFC" && $FOURNISSEUR != "NEWCLOUD" && $FOURNISSEUR != "PM1000" ]]; then
...
EOF
        ...
        # Check svr_sysalt package et desinstallaiton avant le patch CVE, reinstallation sur module step5-alterned-disk
        SYSALT_PKG=$(rpm -qa | grep svr_sysalt)
        if [ $? -eq 0 ]; then
            echo -e "\tsvr_sysalt package found. Trying to remove it:"
            #SYSALT_UNINSTALL=$(rpm -e $SYSALT_PKG >&2)
            SYSALT_UNINSTALL=$(yum -y remove $SYSALT_PKG >&2)
            if [ $? -eq 0 ]; then
                echo -e "\t\tOK"
            else
                #echo "erreur suppression package ${SYSALT_PKG} via la commande :  rpm -e $SYSALT_PKG" >$LOG
                echo "erreur suppression package ${SYSALT_PKG} via la commande :  yum -y remove $SYSALT_PKG" >$LOG
                echo -e "\t\tError"
                exit 99
            fi
            echo -e "\tPackage will be reinstalled in alterned disk part"
        fi
        ...
        then
            ...
            CMD_INSTALL_MICROCODE="yum --enablerepo=* clean all >/dev/null 2>&1 && YUM8=${YUM8} YUM9=${YUM9_REF_CVE4} yum -y --disablerepo=* --enablerepo=*updates-base install microcode_ctl*"
            su -l root -c "$CMD_INSTALL_MICROCODE" > /dev/null 2>&1
            if [ $? -eq 0 ]
            then
                ...
                echo "Installation microcode_ctl KO" > $LOG
        ...
        su -l root -c "YUM9=${YUM9_REF_CVE3} yum -y --disablerepo=* --enablerepo=*updates-base update sssd" > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            echo "sssd update OK"
        else
            #echo "sssd update KO via la commande : YUM9=${YUM9_REF_CVE3} yum -y update sssd" > $LOG
            echo "sssd update KO" > $LOG
...
EOF
        ...
        # Patch CVE + patch sudo
        #CMD_INSTALL_CVE="yum --enablerepo=* clean all && yum -y --disablerepo=* --enablerepo=*updates-base update kernel* microcode* nettle* gnutls* openssl* sudo* > /dev/null"
        su -l root -c "yum --enablerepo=* clean all && yum -y --disablerepo=* --enablerepo=sysref-rhel-${YUM9}-updates-base update kernel* microcode* nettle* gnutls* openssl* sudo*" > /dev/null 2>&1
        if [ $? -eq 0 ]; then
            echo -e "\tPatch CVE OK"
        else
            # echo "Patch CVE KO via la commande : yum -y update kernel* microcode* nettle* gnutls* openssl* sudo*" >$LOG
            echo "Patch CVE KO pour update kernel* microcode* nettle* gnutls* openssl* sudo*" >$LOG
        ...
        if [ $? -eq 0 ]; then
            ...
            echo -e "\tPackage microcode_ctl not found. Trying to install it: "
            #YUM8=${YUM8} YUM9=${YUM9} yum -y install microcode_ctl*
            su -l root -c "yum --enablerepo=* clean all && yum -y --disablerepo=* --enablerepo=sysref-rhel-${YUM9}-updates-base install microcode_ctl*" > /dev/null 2>&1
            if [ $? -eq 0 ]; then
                echo -e "\tInstallation microcode_ctl OK"
            else
                #echo "Installation microcode_ctl KO via la commande : YUM8=${YUM8} YUM9=${YUM9} yum -y install microcode_ctl*" >$LOG
                echo "Installation microcode_ctl KO" >$LOG
                echo -e "\tInstallation microcode_ctl KO " && exit 3
            fi
        fi
    ...
    *)
    ...
    echo "OS, $OS, pas de patch CVE"

else

    echo "OS, $OS, not supported" >$LOG
    echo "OS, $OS, not supported" && exit 97
fi
