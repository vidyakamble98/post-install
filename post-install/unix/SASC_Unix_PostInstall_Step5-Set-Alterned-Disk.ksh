#!/bin/ksh
########################################################################################################################
# Script: SASC_Unix_PostInstall_Step5-Set-Alterned-Disk.ksh
#
...
#                           Piotr Ulaszewski - 08/03/2023 - modification de test pour RHEL8 ou superieur - boom traité dans la partie software
#
...
"Linux")
    ...
    #### Les RHEL 8 utilisent BOOM et non pas sysalt.
    if ! [ "$REL" == "7" ] && ! [ "$REL" == "6" ]; then
        #BOOM est installé dans la partie software
    echo "OS RHEL8 ou superieur - Cette partie sera traité séparement dans la partie software"
    exit 0
