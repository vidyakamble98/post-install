#!/bin/ksh
########################################################################################################################
# Script: SASC_Unix_PostInstall_Step4-Set-Custom-Config.ksh
#
# Description: Ce script permet de configurer des éléments divers sur une machine Unix lors de la post-instalation
# Version: 1.0.7 (ou 1: modification majeure, 2: modification mineure, 3: correction de bug)
#
...
#                           Piotr Ulaszewski - 24/02/2023 - Ligne 195 install ncompress sans creation de fichier de log KO
#                           Piotr Ulaszewski - 30/03/2023 - Modification de test si version RHEL 6 ou 7 pour ajouter le filtre sysalt dans la crontab
#                           Piotr Ulaszewski - 30/03/2023 - AIX - pas de test si le script extract errpt existe, le package exploit isim va etre copie dans une etape suivante
#
...
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        #YUM8=${YUM8} YUM9=${RHE_MIN_VERS} yum -y install ncompress >>$LOG
        YUM8=${YUM8} YUM9=${RHE_MIN_VERS} yum -y install ncompress
        if [ $? -eq 0 ]; then
            echo "${ETAPE} OK"
        else
            echo "${ETAPE} KO" >$LOG
    ...
    7)
        ...
        if [ -f ${Path_Script}${Set_Script} ]; then
...
EOF
        ...
        if [ -f ${Path_Script}${Check_Script} ]; then
...
\
    ...
    if [ $? -eq 0 ]; then
        ...
        #pas de test si le script existe ici, il va etre copie dans une etape suivante avec le package exploit isim
        #if [ -f /exploit/isim/SCRIPTS/SYSTEM/ExtractErrpt/extract_errpt.ksh ]; then
...
EOF
    ...
    # Creation des liens
    if [ -L /etc/init.d/S21${Set_Script} ]; then
    ...
    if [ -L /etc/init.d/K21${Set_Script} ]; then
